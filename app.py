...
def generar_registro_laboratorio(centro_id, output_path, tipo_firma="digital"):
    ...
    def _planta_desde_nivel(nivel_valor):
        """El laboratorio no quiere el texto libre de "Planta" tal cual
        se escribe en la app, sino un valor 0 / -1 según el nivel."""
        if nivel_valor == "Planta Baja":
            return "0"
        if nivel_valor == "Bajo rasante":
            return "-1"
        return nivel_valor or ""
    ...
    def _ubicado_en(zona_valor, nombre_centro):
        ...
        se usa el nombre del propio centro en su lugar."""
        valores_genericos = ("atención primaria", "atencion primaria", "pac")
...
def _renderizar_campos_detector(cid, detector_id, ns):
    ...
    título, sin el botón "Volver" y sin el botón "Guardar"): se usa
    tanto en la pantalla independiente "Nuevo detector" como incrustado
    dentro de "Detectores colocados" al abrir uno ya existente."""
...
def _guardar_detector_desde_ns(cid, detector_id, ns, mostrar_mensajes=True):
    ...
    if not sala_val or not codigo_val:
        if mostrar_mensajes:
            if not sala_val:
    ...
    if st.session_state.mostrar_form_nuevo_centro:
        ...
        if crear:
            if nombre_nuevo and nombre_nuevo.strip():
                cid = crear_centro(nombre_nuevo.strip(), zona_nueva.strip() if zona_nueva else "")
...
def pantalla_centro_detectores():
    ...
    def _cerrar_detector_abierto(autoguardar):
        """Cierra el detector que estuviera desplegado en línea. Si
        autoguardar=True, intenta guardar antes lo que hubiera en sus
        campos (guardado automático al cambiar de detector o salir de
        la pantalla, para no perder cambios sin querer)."""
        anterior = st.session_state.get(abierto_key)
        if anterior is not None:
            ns_anterior = f"det_{anterior if anterior != 'nuevo' else 'nuevo'}"
            det_id_anterior = None if anterior == "nuevo" else anterior
            if autoguardar:
                _guardar_detector_desde_ns(cid, det_id_anterior, ns_anterior, mostrar_mensajes=False)
            _limpiar_namespace(ns_anterior)
        st.session_state[abierto_key] = None

    st.markdown('<div class="marcador-btn-azul-claro"></div>', unsafe_allow_html=True)
    if st.button("← Volver al centro"):
        _cerrar_detector_abierto(autoguardar=True)
        st.session_state.view = "centro"
        st.rerun()
            ...
            with b1:
                ...
                if st.button("📂 Abrir detector", type="primary", use_container_width=True):
                    if st.session_state.get(abierto_key) != did_sel:
                        _cerrar_detector_abierto(autoguardar=True)
                        st.session_state[abierto_key] = did_sel
                        st.session_state["detector_form_ns"] = f"det_{did_sel}"
                    st.rerun()
            with b2:
                st.markdown('<div class="marcador-btn-eliminar"></div>', unsafe_allow_html=True)
            ...
            if st.session_state.get("confirmar_borrado_det") == did_sel:
                ...
                with cc1:
                    if st.button("Sí, eliminar", key=f"conf_del_det_{did_sel}", type="primary"):
                        ...
                        if st.session_state.get(abierto_key) == did_sel:
                            _cerrar_detector_abierto(autoguardar=False)
                        st.rerun()
                with cc2:
    ...
    # aquí mismo debajo del selector en vez de en otra pantalla). Al
    # abrir otro detector, o al salir de esta pantalla, lo que hubiera
        ...
        with st.container(border=True):
            ...
            with gcol1:
                ...
                if st.button("💾 Guardar detector", type="primary", use_container_width=True):
                    guardado_id = _guardar_detector_desde_ns(cid, detector_id_abierto, ns)
                    if guardado_id:
...
def pantalla_centro():
    ...
    # (con su botón de "← Volver al centro" dentro), en vez del acordeón
