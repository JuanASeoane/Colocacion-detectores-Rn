...
st.markdown(
    ...
    /* Botón "Eliminar" (marcado con un div justo antes, ver código
       ...
       vea claramente como una acción de peligro/secundaria. */
    div:has(> div.marcador-btn-eliminar) + div button {
    ...
    div:has(> div.marcador-btn-eliminar) + div button:hover {
        background-color: rgba(224, 90, 90, 0.15) !important;
        border-color: #e05a5a !important;
        color: #e05a5a !important;
        opacity: 1;
    }
    div:has(> div.marcador-btn-eliminar) + div[data-testid="stButton"] {
        display: flex !important;
        justify-content: flex-start !important;
    }

    /* Botón "Generar documentos": lo más pequeño posible, alineado a
       la izquierda (ya va en una columna estrecha), conservando el
       naranja destacado por ser la acción principal. */
    div:has(> div.marcador-btn-generar) + div button {
    ...
    div:has(> div.marcador-btn-generar) + div[data-testid="stButton"] {
        display: flex !important;
        justify-content: flex-start !important;
    }
    ...
    /* Botón "Importar centro": fondo oscuro con las letras en blanco
       (igual que el "+" de "Nuevo centro"). */
    div:has(div.marcador-btn-importar) + div button {
        background-color: #262626 !important;
        border: 1px solid #666666 !important;
        color: #ffffff !important;
    }
    div:has(div.marcador-btn-importar) + div button:hover {
        background-color: #333333 !important;
        border-color: #999999 !important;
        color: #ffffff !important;
    }
    ...
    try:
        ...
        for idx, d in enumerate(detectores, 1):
            ...
            if foto_sit and os.path.exists(foto_sit) and foto_det and os.path.exists(foto_det):
                ...
                story.append(Table([[cap1, cap2], [img1, img2]], colWidths=[8*cm, 8*cm]))
    ...
    with st.container(border=True):
        ...
        with bc1:
            if st.session_state.mostrar_form_nuevo_centro:
                st.markdown('<div class="marcador-activo-naranja"></div>', unsafe_allow_html=True)
        ...
        with st.container(border=True):
            ...
            if st.session_state[mostrar_add_plano_key]:
                ...
                if st.button("Guardar plano", key=f"guardar_plano_{cid}", type="primary"):
                    ...
                    elif not archivo_plano_nuevo:
                        st.warning("Selecciona una imagen para el plano")
                    else:
    ...
    if _cabecera_bloque("📄 Informes y descargas", "informes"):
      with st.container(border=True):
        if not detectores:
            ...
            # --- Un único botón que genera los tres archivos a la vez,
            ...
            with col_gen:
                ...
                generar_clic = st.button(
                    "📦 Generar documentos", type="tertiary", use_container_width=True,
                )
            ...
            if generar_clic:
                try:
                    ...
                    with st.spinner("Generando PDF, hoja de cálculo y fotos..."):
            ...
            ya_generado = st.session_state.get("ultimo_pdf_centro") == cid
            ...
            if ya_generado:
                ...
                # 1) Descargar PDF (con el icono de PDF junto al texto)
                ...
                # 3) Descargar Excel
                if ultimo_excel and os.path.exists(ultimo_excel):
                ...
                # 5) Seleccionar todas + checklist individual de fotos
                seleccionadas = []
                if fotos_disponibles:
                    st.markdown(
                        '<p style="color:#F5A623; font-size:1.05rem; font-weight:700; margin:0.5rem 0 0.3rem 0;">'
                        '📷 Fotos del informe</p>',
                        unsafe_allow_html=True,
                    )
                    ...
                    with sb1:
                        if st.button("☑️ Seleccionar todas", key=f"wa_selall_{cid}", use_container_width=True):
                            for i, foto in enumerate(fotos_disponibles):
                                st.session_state[f"wa_chk_{cid}_{i}_{_slug(foto['nombre_archivo'])}"] = True
                            st.rerun()
                    with sb2:
                        if st.button("⬜ Quitar selección", key=f"wa_deselall_{cid}", use_container_width=True):
                            for i, foto in enumerate(fotos_disponibles):
                                st.session_state[f"wa_chk_{cid}_{i}_{_slug(foto['nombre_archivo'])}"] = False
                            st.rerun()
                ...
                # 6) Descargar fotos
                if ultimo_zip and os.path.exists(ultimo_zip):
                ...
                # 7) Enviar fotos seleccionadas por WhatsApp
                if fotos_disponibles:
