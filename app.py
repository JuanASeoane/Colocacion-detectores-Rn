...
def generar_registro_laboratorio(centro_id, output_path, tipo_firma="digital"):
    ...
    # solape nunca con las últimas filas de la tabla.
    margen_inferior = 3.2*cm if tipo_firma == "digital" else 1.2*cm
    doc = SimpleDocTemplate(
        output_path, pagesize=landscape(A4),
        topMargin=1.2*cm, bottomMargin=margen_inferior, leftMargin=1.2*cm, rightMargin=1.2*cm,
    )
    styles = getSampleStyleSheet()
    story = []
    ...
    codigo_formulario_txt = "CF-PE-CYE/39 V.3 (30/05/2025)"
    estilo_codigo_derecha = ParagraphStyle(
        "CodigoDerecha", parent=styles["Normal"], fontSize=5.5, leading=7,
        alignment=TA_RIGHT, textColor=colors.black,
    )
    celda_orden_trabajo = [
        Paragraph(codigo_formulario_txt, estilo_codigo_derecha),
    ...
    story.append(fila_cabecera_completa)
    story.append(Spacer(1, 0.3*cm))
    ...
    n_filas_totales = max(len(detectores) + 3, 15)
    ...
    # --- Pie de página: cuadro de firma del técnico si la firma es
    # digital, repetido en TODAS las páginas. Se dibuja directamente en
    # el lienzo (no como contenido que fluye tras la tabla) para que
    # nunca "empuje" una página en blanco de más. Con firma a mano no
    # se dibuja nada. ---
    dibujar_pie = None
    if tipo_firma == "digital":
        def _dibujar_caja_firma_digital(canvas_obj, doc_obj):
            from reportlab.lib.units import cm as _cm
            canvas_obj.saveState()
            ancho_caja = 4.0 * _cm
            alto_caja = 1.5 * _cm
            separacion_tabla = 0.4 * _cm
            # Alineada con el borde derecho REAL de la tabla (que no
            # llega hasta el margen de la página, se queda un poco
            # antes), así que no vale usar rightMargin aquí).
            x = doc_obj.leftMargin + 6 + ancho_total - ancho_caja
            y = doc_obj.bottomMargin + separacion_tabla
        ...
        dibujar_pie = _dibujar_caja_firma_digital

    if dibujar_pie:
        doc.build(story, onFirstPage=dibujar_pie, onLaterPages=dibujar_pie)
    else:
        doc.build(story)
